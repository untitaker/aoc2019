...
parseLine [] = []
parseLine (d:rest) = (Step direction number):(parseLine rest')
    where
    direction = read [d]
    number = read $ takeWhile (',' /=) rest
    rest' = drop 1 $ dropWhile (',' /=) rest

[inputLine1, inputLine2] = map parseLine $ lines rawInput

-- calculations

newPoint (x, y) U = (x, y + 1)
newPoint (x, y) D = (x, y - 1)
newPoint (x, y) R = (x + 1, y)
newPoint (x, y) L = (x - 1, y)

steps2Points :: Point -> [Step] -> [Point]
steps2Points p [] = []
steps2Points p (Step _ 0:rest) = steps2Points p rest
steps2Points p (Step s d:rest) = p:(steps2Points (newPoint p s) (Step s (d-1):rest))

-- part 1

intersections = filter (/= (0, 0)) $ Set.toAscList $ pointsA `Set.intersection` pointsB
pointsA = Set.fromList $ steps2Points (0, 0) inputLine1
pointsB = Set.fromList $ steps2Points (0, 0) inputLine2

solution1 = minimum [abs x + abs y | (x, y) <- intersections]

-- part 2

buildSortKey input = (Map.!) $ Map.fromList $ zip (steps2Points (0, 0) input) [0..]
wireStepsA = buildSortKey inputLine1
wireStepsB = buildSortKey inputLine2

solution2 = minimum [wireStepsA p + wireStepsB p | p <- intersections]
